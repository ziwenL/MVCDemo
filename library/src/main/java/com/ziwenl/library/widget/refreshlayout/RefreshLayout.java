...
public class RefreshLayout extends ViewGroup {
    ...
    public boolean isChildNeedTouchEvent = false;//子控件想要触摸事件
    ...
    public boolean canChildScrollUp() {
        if (android.os.Build.VERSION.SDK_INT < 14) {
            if (mScrollChild != null) {
                if (mScrollChild instanceof AbsListView) {
                    final AbsListView absListView = (AbsListView) mScrollChild;
                    return absListView.getChildCount() > 0
                            && (absListView.getFirstVisiblePosition() > 0 || absListView.getChildAt(0)
                            .getTop() < absListView.getPaddingTop());
                } else {
                    return mScrollChild.getScrollY() > 0;
                }
            } else {
            ...
            return ViewCompat.canScrollVertically(mScrollChild == null ? mTarget : mScrollChild, -1);
        }
    }
//    public boolean canChildScrollUp() {
//        if (android.os.Build.VERSION.SDK_INT < 14) {
...
//    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (mDirection) {
            ...
            case TOP:
            default:
                if (!isEnabled() || mReturningToStart || (!mBothDirection && canChildScrollUp()) || mRefreshing || isChildNeedTouchEvent) {
                    // Fail fast if we're not in a state where a swipe is possible
                    return false;
                }
                break;
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                //add ziwenl 解决左右滑动时偶尔会引发下拉动作的问题
                startX = ev.getX();
                startY = ev.getY();
...
//    public boolean isCanPullDown;//是否可以下拉刷新
//    public boolean isCanPullUp;//是否可以上拉加载
